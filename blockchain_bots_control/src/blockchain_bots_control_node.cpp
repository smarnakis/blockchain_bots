...
class TestServiceClient : public rclcpp::Node {
  ...
  public:
    TestServiceClient(const std::string & node_name, std::int32_t robot_id, std::string host, std::int32_t port) : Node(node_name)
    {
      robot_id_ = robot_id;
      host_ = host;
      port_ = port;
...
int main(int argc, char * argv[])
{
  ...
  auto service_client1 = std::make_shared<TestServiceClient>("test_service_client0", 0, "127.0.0.1", 9000);
